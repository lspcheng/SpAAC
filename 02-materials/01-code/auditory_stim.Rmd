...
read_table_transposed <- function(table_file, measure){
  table_df <- read.table(table_file, header=T) %>%
    ...
    mutate(speaker_dir = str_extract(table_file, "(S\\d{2})"))
}
...
read_table_ratings <- function(table_file){
  table_df <- read.table(table_file, header=T) %>%
    rename(filename = Item, rating = Rating) %>%
    mutate(rater = str_extract(table_file, "(R\\d{2})"), .before=rating)
}
...
## Basic Info
Must re-run this code to get the correct number of reference rows per speaker, given that some speakers don't have the full 160 tokens.
```{r}
recordings_path <- file.path(PROJECT_DIR, "02-materials", "02-stimuli", "P0-norming", "n2", "03-recordings")

token_info <- list.files(recordings_path,
...
```{r}
# Read in data
w_dur_df <- list.files(recordings_path,
...
w_dur_df %>%
  ...
  ggsave(filename=paste("All", "duration-word", "profile.png", sep="_"), path=file.path(pipeline, "temp"), width=10, height=6)
...
for (spk_num in speaker_list){
  ...
  print(plot)
  ggsave(filename=paste(spk_num, "duration-word", "profile.png", sep="_"), path=file.path(pipeline, "temp"), width=10, height=6)
}
```
...
### Mean
```{r}
w_meanintensity_df <- list.files(recordings_path,
...
### Normalized
```{r}
w_normtimeIntensity_df <- list.files(recordings_path,
...
### Mean
```{r}
w_meanf0_df <- list.files(recordings_path,
...
### Max
```{r}
w_maxf0_df <- list.files(recordings_path,
...
### Min
```{r}
w_minf0_df <- list.files(recordings_path,
...
### Final
```{r}
w_finalf0_df <- list.files(recordings_path,
...
### Normalized
```{r}
w_normf0_df <- list.files(recordings_path,
...
for (spk_num in speaker_list){
  ...
  # Across all tokens
    ...
    # stat_summary(geom="col", alpha=0.5, position="dodge") +
    stat_summary(fun.data = mean_se, geom="pointrange", position=position_dodge(0.95)) +
    stat_summary(fun.data = mean_se, geom="smooth", position=position_dodge(0.95)) +
    scale_x_continuous(limits = c(2, 9), breaks=2:9) +
  ...
  plot <- w_normf0_df %>% filter(speaker==spk_num) %>%
    ...
    # geom_point(alpha=0.2) +
    # stat_summary(geom="col", alpha=0.5, position="dodge") +
    stat_summary(fun.data = mean_se, geom="pointrange", position=position_dodge()) +
    stat_summary(fun.data = mean_se, geom="smooth", position=position_dodge()) +
    scale_x_continuous(limits = c(2, 9), breaks=2:9) +
    labs(title = paste0(spk_num))
   print(plot)
 ...
 print(plot)
 # ggsave(filename=paste(spk_num, "f0", "profile.png", sep="_"), path=file.path(pipeline, "temp"), width=10, height=6)
}
...
```
#### By-Row for Speaker+Variable
```{r}
current_speaker = speaker_list[[3]]
current_var = 8

for (row_num in filter(w_normf0_df, speaker==current_speaker & variableN==current_var) %>% pull(rowN) %>% unique() 
     ){
  plot <-  w_normf0_df  %>% filter(speaker==current_speaker) %>%
    filter(variableN==current_var) %>% filter(rowN==row_num) %>% 
  ggplot(aes(x=timepoint, y=normf0, col=variant)) + gg_theme() + 
  facet_wrap(~rowN, ncol = 5) +
  geom_point() +
  geom_line() +
  ...
  labs(title=paste0("Variable ", current_var))
  print(plot)
}
```
...
```{r}
# Read in data
p_dur_df <- list.files(recordings_path,
           pattern="duration.txt",  recursive=TRUE, full.names = TRUE) %>%
...
### Mean
```{r}
p_meanintensity_df <- list.files(recordings_path,
           pattern="meanintensity.txt",  recursive=TRUE, full.names = TRUE) %>%
  stringr::str_subset(., "phone_level", negate = FALSE) %>%
   plyr::ldply(., read_table_transposed, "meanintensity") %>%
  cbind(token_info, .) %>% 
...
### Normalized
```{r}
p_normform_df <- list.files(recordings_path,
...
#### By-Variable for Speaker
```{r, fig.width=12, fig.height=6}
current_speaker = speaker_list[[6]]
...
for (var_num in variable_list){
  ...
  plot1 <- plot_df %>%
    ...
    stat_summary(fun.data = mean_se, geom="pointrange", position=position_dodge()) +
    stat_summary(fun.data = mean_se, geom="line", position=position_dodge()) +
    # stat_summary(geom="smooth", position=position_dodge()) +
    scale_x_continuous(limits = c(1, 9), breaks=1:9) +
    labs(title="Average across Tokens")
...
## [C] Auditory Ratings
For the consonant ratings (V3, V8), ratings of 1 = more mainstream, 3 = ambiguous, 5 = non-mainstream. 
So, tokens coded as variant M should be rated close to 1 and N should be rated close to 5.
...
For the /ou/ word-level emphasis ratings (V4), ratings of 1 = less emphasized, 3 = neutral, 5 = emphasized.
So, tokens should ideally all be rated close to 3 (neutral). If there are differences due to fronting/backing inherently, we might see fronted M variants having ratings shifted towards 1 (e.g. 2~2.5) and backed N variants having ratings shifted towards 5 (e.g. 3.5~4).
...
```{r}
# Read in data
p_rating_df <- list.files(recordings_path,
           pattern="ratings.txt",  recursive=TRUE, full.names = TRUE) %>%
  stringr::str_subset(., "1_manual", negate = FALSE) %>%
   plyr::ldply(., read_table_ratings) %>%
  inner_join(token_info, .)
p_rating_df
```
...
# Plot grand means
p_rating_df %>%
  ggplot(aes(x=variant, y=rating)) + gg_theme() +
  geom_violin(width=0.5, position=position_dodge(0.95)) +
  stat_summary(fun = mean, geom="point", position=position_dodge(0.25)) +
  stat_summary(fun.data = mean_se, geom="errorbar", position=position_dodge(0.25), width=0.25)

# Plot means across speaker per variable
p_rating_df %>%
  ggplot(aes(x=variable, y=rating, linetype=variant, color=variable)) + gg_theme() +
  geom_violin(width=0.5, position=position_dodge(0.95)) +
  stat_summary(fun = mean, geom="point", position=position_dodge(0.25)) +
  stat_summary(fun.data = mean_se, geom="errorbar", position=position_dodge(0.25), width=0.25)
...
# Plot by-speaker means per variable
for (spk_num in speaker_list){
  plot <- p_rating_df %>% filter(speaker==spk_num) %>%
  ggplot(aes(x=variable, y=rating, linetype=variant, color=variable)) + gg_theme() +
    facet_wrap(~speaker) +
  geom_violin(width=0.5, position=position_dodge(0.95)) +
  stat_summary(fun = mean, geom="point", position=position_dodge(0.25)) +
...
# Standardized output plot
p_rating_df %>%
  ggplot(aes(x=variable, y=rating, linetype=variant, color=variant)) + gg_theme() +
  geom_violin(width=0.4, position=position_dodge(0.95)) +
  stat_summary(fun = mean, geom="point", position=position_dodge(0.25)) +
  stat_summary(fun.data = mean_se, geom="errorbar", position=position_dodge(0.25), width=0.1) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(title= "All Speakers", x="Variable", y="Rating (1-5)")
  # ggsave(filename=paste("All", "rating-phone", "profile.png", sep="_"), path=file.path(pipeline, "temp"), width=10, height=6)
...
for (spk_num in speaker_list){
  ...
  labs(title= paste0(spk_num), x="Variable", y="Word Duration (ms)")
  print(plot)
  # ggsave(filename=paste(spk_num, "rating-phone", "profile.png", sep="_"), path=file.path(pipeline, "temp"), width=10, height=6)
}
```
...
p_df <-
  p_dur_df %>%
  full_join(p_meanintensity_df) %>%
  full_join(ft_normform_df %>%
              pivot_wider(1:10,
                          names_from = c(formant, timepoint),
                          values_from = c(normform_raw, normform_logmean),
                          names_glue = "{.value}_{formant}_T{timepoint}"
                          ),
            ) %>% rename_with(~ gsub("normform_", "", .)) %>%
  mutate(variable = fct_relevel(variable, "PIN-PEN", "FEEL-FILL",
                                "TH-stopping", "OU-backing", "U-fronting",
                                "AEN-raising", "AE-backing", "T-deletion"))
p_df
...
# Simplified
p_summary_df <-
  p_df %>% select(-variableN:-variantN) %>% group_by(variable, variant) %>%
  # select(!ends_with("T1") & !ends_with("T10")) %>%
  summarize(across(where(is.numeric), ~ round(mean(.x), 1))) %>%
...
## Final Choices
```{r}
# Copy from pipeline out to 05-output
# file.path("...", "05-output", "01-figures", "materials", "stimuli")
```
