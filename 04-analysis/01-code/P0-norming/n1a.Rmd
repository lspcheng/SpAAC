...
NAME <- 'n1a' ## Name of the R file (w/o file extension!)
...
```{r}
n1a_screener_data <- read_csv(final_screener) %>%
...
n1a_screener_v_prolific <-
  n1a_screener_data %>% full_join(n1a_prolific_demo, by="Participant id")  %>% 
  select(
    Run,
    `Participant id`,
    Time_in_min,
...
n1a_screener_v_prolific %>% arrange(`Participant id`)
```
...
```{r}
participant_list <- n1a_screener_v_prolific %>% filter(Run=="finalv2") %>%  pull(`Participant id`)
participant_list
```
...
n1a_screener_data %>%  filter(PROLIFIC_PID == current_participant) #Prolific_ID

n1a_screener_v_prolific %>%  filter(`Participant id` == current_participant) #Prolific_ID
...
n1a_data <- read_csv(final_data_num)  %>% 
...
n1a_data_text <- read_csv(final_data_text)  %>% 
  # NOTE: Unfinished data doesn't appear in the export file until X days later. That's why the data may seem to change if I export new data later.
...
n1a_codes <- read.csv(file.path(stim_data_path, NAME, "02-records","SpAAC Visual Stimuli - N1_PseudoRanStim_Long.csv"))
n1a_codes
```
...
## Check a participant
# n1a_subj_cond %>%  filter(PROLIFIC_PID=="596fc4717008ef000109703d")
```
...
```{r}
# View
(n1a_subj_p1 <- n1a_subj %>% select(PROLIFIC_PID, WhatTaskAbout:TaskNotice))
```
Process Part 2 responses about the personal demographics.
```{r}
n1a_subj_p2 <- n1a_subj %>% select(PROLIFIC_PID, Age:Education) %>%
  na_if(., -99) %>%
...
n1a_subj_p2 %>% full_join(n1a_subj_cond, .) %>%
...
n1a_subj_p2 %>% filter(PROLIFIC_PID == current_participant)

# View locations by subject
n1a_subj_p2_loc %>%  filter(PROLIFIC_PID == current_participant) %>%
...
```{r}
n1a_responses <- n1a_data %>%
  slice(2:nrow(.)) %>%
  select(PROLIFIC_PID, '1_Q26.1_1':'9_Q79.1_DO_13') %>% 
  pivot_longer('1_Q26.1_1':'9_Q79.1_DO_13', names_to = c("Loop", "Q", "ChoiceID"), names_sep="_", values_to = "Response") %>%
  filter(ChoiceID!="DO") %>%
  full_join(., n1a_response_info) %>%
...
n1a_responses_selected <- n1a_responses %>%
  select(PROLIFIC_PID, Descriptor_Set, Descriptor, Image_Cat, Image, Response) %>%
  mutate(across(where(is.character), as.factor))
n1a_responses_selected
# View(n1a_responses_selected)
summary(n1a_responses_selected)
```
...
current_descriptors <- n1a_responses_selected %>% filter(PROLIFIC_PID == current_participant) %>% pull(Descriptor) %>% unique() # pull() returns vector
current_responses <- n1a_responses_selected %>% filter(PROLIFIC_PID == current_participant) %>%
...
n1a_props_byImage <- n1a_responses_selected %>%
...
n1a_scores_byImage <- n1a_props_byImage %>% 
  # Get mean values
...
score_vars_cor <- 
  n1a_scores_byImage %>% select(-Image_Cat, -Image) %>% 
...
```{r}
## Maximal set
image_score_vars_all <- n1a_scores_byImage %>% select(-Image_Cat, -Image)
...
n1a_PCAscores_byImage <- n1a_scores_byImage %>%
  cbind(
...
n1a_PCAscores_byImage
```

```{r}
# Check how PCs map onto raw scores
# Which photos were selected as FrCA over 50% of the time
n1a_PCAscores_byImage %>% select(Image_Cat, Image, Dim3_NonCalifornian, FrCA) %>% arrange(-FrCA) %>% filter(FrCA>0.5) #%>% summarize(max_Dim3 = max(Dim3_NonCalifornian)) # Max Dim3 = 0.6065949
n1a_PCAscores_byImage %>% select(Image_Cat, Image, Dim3_NonCalifornian, FrCA) %>% arrange(Dim3_NonCalifornian)

# Which photos were selected as WhAm over 50% of the time
n1a_PCAscores_byImage %>% select(Image_Cat, Image, Dim1_White, AsAm, WhAm, EthCul, AmCul, FrCA) %>% arrange(-WhAm) %>% filter(WhAm>0.5) #%>% summarize(min_Dim1 = min(Dim1_White)) # Min Dim1 = 0.2245709	
# Which photos were selected as AsAm 
n1a_PCAscores_byImage %>% select(Image_Cat, Image, Dim1_White, AsAm, WhAm, EthCul, AmCul, FrCA) %>% arrange(-AsAm)

# Which photos were selected as AsAcc
n1a_PCAscores_byImage %>% select(Image_Cat, Image, Dim2_American, AmAcc, ForAcc, AmCul, EthCul, FrCA) %>% arrange(-AmAcc) %>%
  #filter(AmAcc>0.5) %>% 
...
n1a_PCAscores_byImage %>% #filter(Image_Cat=="AAW") %>%
...
```{r}
# Dim3 not visualized 
n1a_PCAscores_byImage %>% #summarize(max=max(Dim3_NonCalifornian), min=min(Dim3_NonCalifornian))
...
##### a) Mainstream White
```{r}
msWhite_images <- n1a_PCAscores_byImage %>%
...
##### b) Mainstream Asian
```{r}
msAsian_images <- n1a_PCAscores_byImage %>%
...
```{r}
ethAsian_images_all <- n1a_PCAscores_byImage %>%
...
n1a_PCAscores_screenedImages <-
  ...
  mutate(Persona_Race = ifelse(Persona=="msWhite", "White", "Asian"),
         ...
         .after=Persona)
n1a_PCAscores_screenedImages
write_csv(n1a_PCAscores_screenedImages, file=file.path(pipeline, "out", "n1a_PCAscores_screenedImages.csv"))
```
...
n1a_PCAscores_selectedImages <-
  ...
  mutate(Persona_Race = ifelse(Persona=="msWhite", "White", "Asian"),
         ...
         .after=Persona)
n1a_PCAscores_selectedImages
write_csv(n1a_PCAscores_selectedImages, file=file.path(pipeline, "out", "n1a_PCAscores_selectedImages.csv"))
...
personae_means <- n1a_PCAscores_selectedImages %>%
