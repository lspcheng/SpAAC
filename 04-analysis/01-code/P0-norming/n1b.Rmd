...
n1b_codes <- read.csv(file.path(stim_data_path, NAME, "02-records","SpAAC Visual Stimuli - N1b_Stims.csv")) %>%
  mutate(photo_num = seq(1:n()), .before="Critical.Filler")
n1b_codes
```
...
# Total presented/answered photos (end goal: 600)
n1b_response_data_clean %>% select(subj, photo_num) %>% distinct() %>% count()
# Total presented/answered photos per participant (process/end goal: 10 per subject)
n1b_response_data_clean %>% select(subj, photo_num) %>% distinct() %>% count(subj)
# Total presented/answered participants per photo (end goal: 20 per photo)
n1b_response_data_clean %>% select(subj, photo_num) %>% distinct() %>% count(photo_num) %>% type_convert() %>% arrange(photo_num)
...
n1b_subj_cond <- n1b_subj %>% 
  ...
  full_join(
    ...
    pivot_longer(cols=2:last_col(), names_to=c(NA, NA, "photo_num"),
...
NOTE: Can drop main_q and sub_q if not useful as shorthand.
...
n1b_responses <- n1b_data %>%
  ...
  slice(-1) %>% # remove second header
  pivot_longer(cols=2:last_col(), names_to=c("photo_num", "main_q", "sub_q", NA), names_sep="_", values_to = "Response") %>%
...
# Total presented/answered photos (end goal: 600)
n1b_responses %>% select(PROLIFIC_PID, photo_num) %>% distinct() %>% count()
# Total presented/answered photos per participant (process/end goal: 10 per subject)
n1b_responses %>% select(PROLIFIC_PID, photo_num) %>% distinct() %>% count(PROLIFIC_PID)
# Total presented/answered participants per photo (end goal: 20 per photo)
n1b_responses %>% select(PROLIFIC_PID, photo_num) %>% distinct() %>% count(photo_num) %>% arrange(photo_num)
...
n1b_responses_selected <- n1b_responses %>%
  select(PROLIFIC_PID, Condition, Image_Cat, Image, Response) %>%
  mutate(across(where(is.character), as.factor))
...
current_responses <- n1b_responses %>% 
  select(PROLIFIC_PID, main_label, sub_label, Image, Response) %>% 
  filter(PROLIFIC_PID == current_participant) %>%
  mutate(Response = case_when(main_label=="Recognize" & Response=="2" ~ "No",
                              main_label=="Recognize" & Response=="1" ~ "Yes",
                              TRUE ~ Response)) 

current_responses %>% filter(main_label=="Impressions") %>% pivot_wider(id_cols = c(PROLIFIC_PID, Image), names_from = sub_label, values_from = Response)

# Non-numeric responses
current_responses %>% filter(main_label=="Age") %>%
  filter(Response!=0) %>% select(PROLIFIC_PID, Image, sub_label)
  # pivot_wider(id_cols = c(PROLIFIC_PID, Image), names_from = sub_label, values_from = Response)

current_responses %>% filter(main_label=="Occupation") %>% select(-contains("sub_label"))

current_responses %>% filter(main_label=="GrewUp") %>% select(-contains("sub_label"))

current_responses %>% filter(main_label=="Ethnicity") %>% select(-contains("sub_label"))

current_responses %>% filter(main_label=="Speech") %>% select(-contains("sub_label"))

current_responses %>% filter(main_label=="Recognize") %>% select(-contains("sub_label"))

```

```{r}
# Rating responses
current_ratings <- current_responses %>% filter(main_label=="Ratings") %>% select(-contains("main_label")) %>% select(-PROLIFIC_PID) %>% type_convert()

current_responses %>% filter(main_label=="Ratings") %>% select(-contains("main_label")) %>%
  filter(sub_label == str_match(sub_label, ".*American:.*")) %>% select(Image, Scale=sub_label, Response)
...
current_ratings %>% group_by(sub_label) %>% summarize(min=min(Response), max=max(Response), mean=mean(Response), median=median(Response))
...
```{r}
n1b_descriptors <-
  n1b_responses %>% select(PROLIFIC_PID, main_label, sub_label, Image, Response) %>% 
  filter(main_label=="Impressions") %>%
  select(- PROLIFIC_PID:-sub_label) %>%
  # text cleaning
  mutate(Response = tolower(Response)) %>%
  mutate(Response = gsub("-", "", Response, fixed=TRUE)) %>%
  # get counts
