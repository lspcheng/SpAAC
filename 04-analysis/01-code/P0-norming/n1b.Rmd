...
```{r}
# linear model
lm_out <- lm(Dim1_American ~ Condition, data=n1b_ratings_vars_pca)
summary(lm_out)
( lm_df <- tidy(lm_out) )

# get marginal averages (estimated marginal means / Least-squares means)
emms <- emmeans(lm_out, "Condition")
( emms_df <- tidy(emms, conf.int = TRUE) )

# get contrasts
# tidy(contrast(emms)) # contrast from grand mean(?)
tidy(pairs(emms))  # contrast between paired groups; equivalent to: contrast(emms, method="pairwise")

# get effect sizes
eff_size(emms, sigma = sigma(lm_out), edf = 27)

# plot confidence intervals
#Option 1
plot(emms, comparisons = TRUE)
#Option 2
ggplot(emms_df, aes(Condition, estimate)) +
...
```{r}
# linear model
lm_out <- lm(Dim2_Likeable ~ Condition, data=n1b_ratings_vars_pca)
summary(lm_out)
( lm_df <- tidy(lm_out) )
...
```{r}
summary(lm(Feminine ~ Condition, data=n1b_ratings_vars_pca))
```
```{r}
summary(lm(Intelligent ~ Condition, data=n1b_ratings_vars_pca))
```
```{r}
summary(lm(Nerdy ~ Condition, data=n1b_ratings_vars_pca))
```

```{r}
summary(lm(Casual ~ Condition, data=n1b_ratings_vars_pca))
```
