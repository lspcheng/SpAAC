...
```{r}
# Mean Bar plots
n1b_asian_network_long <- n1b_asian_network %>% pivot_longer(2:last_col(), names_to = "measure", values_to = "proportion", names_prefix = "prop_") %>%
  mutate(measure = fct_relevel(measure, "Asian", "AsianCC", "AsianRC", "AsianBC"))

n1b_asian_network_long %>% filter(measure!="AsianLocalCC") %>%
  summary_mean_plot(x=measure, y=proportion, full_scale = TRUE, scale_steps = 0.25) + labs(title="Overall Asian Network/Experience")

n1b_asian_network_long %>% filter(measure %in% c("AsianCC", "AsianLocalCC")) %>%
  summary_mean_plot(x=measure, y=proportion, full_scale = TRUE, scale_steps = 0.25) + labs(title="Overall Asian Network/Experience")
```
...
n1b_subj_info <- n1b_subj_p2 %>% 
  ...
  # Add in asian network scores, filling NA with zeros
   full_join(n1b_asian_network) %>% mutate(across(starts_with("prop"), ~ ifelse(is.na(.x), 0, .x)))
n1b_subj_info
save(n1b_subj_info, file=file.path(pipeline, "store", "n1b_subj_info.RData"))

# NOTE: There is additional multirow per-participant dataframes (excluded from merging)
# n1b_subj_p2_loc # Location
# n1b_subj_p2_network # Network scores across all ethnicities
```
...
# View(n1b_responses)
save(n1b_responses, file=file.path(pipeline, "temp", "n1b_responses.RData"))
