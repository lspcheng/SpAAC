...
quick_summarize <- function(df, col, na.rm=FALSE){
  ...
  df %>%  summarize(across(!!col, list(median= ~ median(.x, na.rm=na.rm), mean= ~ mean(.x, na.rm=na.rm), 
                                       sd= ~ sd(.x, na.rm=na.rm), min=min, max=max)))
}
...
main_screener <- file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions +Screener_June+1,+2023_09.17.csv")
main_data_num <-
  file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions_June+1,+2023_09.18_num.csv")
main_data_text <-
  file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions_June+1,+2023_09.18_text.csv")
...
n1b_screener_v_prolific %>% count(`Participant id`)
```
...
```{r}
current_participant <- "62825aef9d9d5544867a6fc0" #participant_list[1] #"5f7d4a2c5db79d21c7d07240"
...
current_participant <- "62825aef9d9d5544867a6fc0" # OR e.g. participant_list[3]
...
```{r}
current_participant <- "62825aef9d9d5544867a6fc0" #participant_list[10] # <- or, REPLACE with PROLIFIC_PID string
...
n1b_grewup <-
  ...
  mutate(Response = mgsub(Response, c("philipinnes|phillippines", "america|^us$", "(?:^|\\W)ca(?:$|\\W)", "wisconson"), c("philippines", "usa", "california", "wisconsin"))) %>% 
...
# Add categorization into accentedness
  ...
  mutate(Accented = case_when(
    str_detect(Response, "(?:^|\\W)(accented|with a(.+\\s)accent|(aapi|hawaiian|asian) accent|broken english)(?:$|\\W)") ~ "Accented",
    TRUE ~ NA
  )) %>%
  mutate(Unaccented = case_when(
    str_detect(Response, "(?:^|\\W)(standard|unaccented|without an accent|no accent|(american|canadian|california) accent|(california) style|normal)(?:$|\\W)") ~ "Unaccented",
...
relocate(Condition:Image,
       Dim1_American=Dim.1, Dim2_Likeable=Dim.2, Dim3=Dim.3
       ) %>% select(-Item)
n1b_ratings_vars_pca
```
...
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim2_Likeable, y=Dim3, label=Image, group=Condition, show_points=FALSE)
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim1_American, y=Dim3, label=Image, group=Condition, show_points=FALSE)
...
scores_df <- n1b_ratings_vars_pca %>% select(-Condition:-Image) %>% 
  select(Dim1_American, Dim2_Likeable, Dim3) #, Feminine, Casual, Intelligent, Nerdy, `Clear speaker`)
...
scores_df <- n1b_ratings_vars_pca %>% select(-Condition:-Image) %>% 
  select(Dim2_Likeable, Dim3) #, Feminine, Casual, Intelligent, Nerdy, `Clear speaker`)
index_df <- n1b_ratings_vars_pca %>% select(Condition:Image)
