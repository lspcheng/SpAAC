...
# Set File paths
RUN <- "1-1_pilot"
pilot_prolific <- file.path(int_data_path, NAME, RUN, "prolific_export_638fb8ef63b505b75b7a60e2.csv")
pilot_screener <- file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions +Screener_May+30,+2023_08.29.csv")
pilot_data_num <- file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions_May+30,+2023_08.30.csv")
pilot_data_text <- file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions_May+30,+2023_08.30(1).csv")
...
main_screener <- file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions +Screener_June+4,+2023_08.43.csv")
main_data_num <-
  file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions_June+4,+2023_09.11.csv")
main_data_text <-
  file.path(int_data_path, NAME, RUN, "Visual+Style+Impressions_June+4,+2023_09.11(1).csv")

# # File paths for: RUN <- "1-3_final"
# final_prolific <- file.path(int_data_path, NAME, RUN, "prolific_export_639cfab1514594c1843130f3.csv")
# # final_screener <- file.path(int_data_path, NAME, RUN, "Visual+Style+Selection +Screener_January+6,+2023_08.27.csv")
# # final_data_num <- file.path(int_data_path, NAME, RUN, "Visual+Style+Selection_January+6,+2023_08.30.csv")
# # final_data_text <- file.path(int_data_path, NAME, RUN, "Visual+Style+Selection_January+6,+2023_08.30(1).csv")
...
n1b_prolific_demo <- 
  read_csv(pilot_prolific) %>% mutate(Run=RUN_LIST[1]) %>%
  full_join(read_csv(main_prolific_1) %>% mutate(Run=paste(RUN_LIST[2]))) %>%
  full_join(read_csv(main_prolific_2) %>% mutate(Run=RUN_LIST[2])) %>%
  full_join(read_csv(main_prolific_3) %>% mutate(Run=RUN_LIST[3])) %>%
...
# Remove Run==NA rows, where no Prolific data (i.e., generally, returned participants)
...
```{r}
current_participant <- "5bf116a61e78580001dc4842" #participant_list[1] #"5f7d4a2c5db79d21c7d07240"
...
n1b_subj_p2_network <- n1b_subj_p2 %>% select(PROLIFIC_PID, contains("Community") | contains("Contacts")) %>%
  ...
  pivot_wider(id_cols=c(PROLIFIC_PID, Ethnicity), names_from = Question, values_from = Response) %>% type_convert()
...
current_participant <- "5dbf692c2a1dd63f64ef5ae4" # OR e.g. participant_list[3]
...
```{r}
current_participant <- "5dbf692c2a1dd63f64ef5ae4" #participant_list[10] # 
...
n1b_grewup <-
  ...
  mutate(Response = mgsub(Response, c("philipinnes|phillippines", "america|^us$|^united states$", "(?:^|\\W)ca(?:$|\\W)", "(?:^|\\W)ny(?:$|\\W)", "wisconson"), c("philippines", "usa", "california", "new york", "wisconsin"))) %>% 
  # Add categorization into US or Asia or Europe
  mutate(Region = case_when(
    Response %in% c("san francisco", "los angeles", "boston", "beverly hills") ~ "USA",
    str_detect(Response, paste0("(?:^|\\W)", paste(tolower(state.name), collapse = '|'), "(?:$|\\W)")) ~ "USA",
    str_detect(Response, "(?:^|\\W)(mid(\\s*)west|(east|west) coast|the west|(west|east)ern states)(?:$|\\W)") ~ "USA", 
...
n1b_ethnicity <-
  ...
  mutate(Response = mgsub(Response, c("china|chiense", "flippino"), c("chinese", "filipino"))) %>% 

  # Add categorization into race
  mutate(Asian = case_when(
    str_detect(Response, "(?:^|\\W)(asian|chinese|filipino)(?:$|\\W)") ~ "Asian",
    str_detect(Response, "(?:^|\\W)(hong kong|singapore|japan|korean|taiwan|vietnam|thai|hmong|malaysian)") ~ "Asian",
    TRUE ~ NA
  )) %>%
...
# Add categorization into accentedness
  mutate(OtherAccent = case_when(
    str_detect(Response, "(?:^|\\W)((english|southern|mid(\\s*|-*)west(ern)*) accent)(?:$|\\W)") ~ "OtherAccent",
    TRUE ~ NA
  )) %>%
  mutate(Accented = case_when(
    str_detect(Response, "(?:^|\\W)(accented|(with|have) a(.+\\s)accent|(aapi|hawaiian|asian) accent|broken english)(?:$|\\W)") ~ "Accented",
    TRUE ~ NA
  )) %>%
  mutate(Unaccented = case_when(
    str_detect(Response, "(?:^|\\W)(standard|unaccented|without an accent|no accent|(american|canadian|california|neutral) accent|(california) style|normal)(?:$|\\W)") ~ "Unaccented",
...
relocate(Condition:Image,
       Dim1_American=Dim.1, Dim2_Likeable=Dim.2, Dim3_Feminine=Dim.3, Dim4_Nerdy=Dim.4
       ) %>% select(-Item)
n1b_ratings_vars_pca
```
...
quick_summarize(n1b_ratings_vars_pca, Dim3_Feminine)
quick_summarize(n1b_ratings_vars_pca, Dim4_Nerdy)
```
...
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim1_American, y=Dim3_Feminine, label=Image, group=Condition, show_points=FALSE)
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim1_American, y=Dim4_Nerdy, label=Image, group=Condition, show_points=FALSE)

n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim2_Likeable, y=Dim3_Feminine, label=Image, group=Condition, show_points=FALSE)
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim2_Likeable, y=Dim4_Nerdy, label=Image, group=Condition, show_points=FALSE)

n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim3_Feminine, y=Dim4_Nerdy, label=Image, group=Condition, show_points=FALSE)
...
scores_df <- n1b_ratings_vars_pca %>% select(-Condition:-Image) %>% 
  select(Dim1_American, Dim2_Likeable, Dim3_Feminine, Dim4_Nerdy) #, Feminine, Casual, Intelligent, Nerdy, `Clear speaker`)
...
scores_df <- n1b_ratings_vars_pca %>% select(-Condition:-Image) %>% 
  select(Dim2_Likeable, Dim3_Feminine, Dim4_Nerdy) #, Feminine, Casual, Intelligent, Nerdy, `Clear speaker`)
index_df <- n1b_ratings_vars_pca %>% select(Condition:Image)
...
```{r}
# (1) LM summary, (2) tidy LM, (3) EMMs w/ CIs, (4) Pairwise comp, (5) Effect sizes*, (6) EMM comp plot, (7) EMM CI plot
lm_emms_pairs(n1b_ratings_vars_pca, Dim3_Feminine ~ Condition)
```
#### Dim4
```{r}
# (1) LM summary, (2) tidy LM, (3) EMMs w/ CIs, (4) Pairwise comp, (5) Effect sizes*, (6) EMM comp plot, (7) EMM CI plot
lm_emms_pairs(n1b_ratings_vars_pca, Dim4_Nerdy ~ Condition)
```
