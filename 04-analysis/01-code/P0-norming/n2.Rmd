...
During data collection, this notebook can generally just be run automatically (Run All) once new data are downloaded and placed in the correct location in the appropriate format (though manual uncommenting will be needed across the stages of data collection.)

Key:
(*) = Manual updates when running data from next data collection Stage (pilot --> main --> final)
(*C) = Manual check prior to approving submissions on Prolific
(*R) = Manaal check for randomization processes, to be referenced for resetting counts if skewed.
...
NAME <- 'n2' ## Name of the R file (w/o file extension!)
...
CURRENT_RUN <-  "final" # RUN_LIST[2] # To get last value: RUN_LIST[length(RUN_LIST)]
```
...
# # pilot_prolific <- list.files(file.path(int_data_path, NAME, RUN),  pattern="prolific_export", full.names = TRUE)
# # pilot_screener <- list.files(file.path(int_data_path, NAME, RUN),  pattern="+Screener", full.names = TRUE)
# test_data_num <- list.files(file.path(int_data_path, NAME, RUN),  pattern="_num", full.names = TRUE) 
# test_data_text <- list.files(file.path(int_data_path, NAME, RUN),  pattern="_text", full.names = TRUE) 
```
...
# n2_data <- read_csv(test_data_num)  %>%
#   filter(Finished!=0) %>%
...
# n2_data_text <- read_csv(test_data_text)  %>%
...
# n2_data %>%
#   select(contains("Q2.X")) %>% select(contains("DO")) %>% slice(-1) %>%
#   mutate(subj=1:n(), .before=1) %>% # Add temp subj number for simulated data
#   pivot_longer(cols=2:last_col(), names_to=c(NA, NA, "Voice_Number"), names_sep="_", values_to = "DO") %>% type.convert() %>%
#   arrange(subj, DO) %>% drop_na(DO) %>%
#   pivot_wider(subj, names_from = DO, names_prefix = "Trial", values_from = Voice_Number)
...
# q_labels <-
#   n2_data %>%
#   select(contains("_Q3.")) %>%   select(-contains("DO")) %>% # select only looped trials, excl, DisplayOrder
#      slice(1) %>%
#   pivot_longer(cols=everything(), names_to=c("Voice_Number", "Question_Number", "Question_Part_Number", NA), names_sep="_", values_to = "q_label") %>%
# #   # Since number of dashes and <> location aren't consistent, need to remove everything between https:// and extra -
#   mutate(q_label = gsub("https://.*? - ", "", q_label)) #%>%
...
```
```{r}
# q_labels %>% pull(Question_Number) %>% unique()
```
```{r}
# q_labels %>% pull(q_label) %>% unique()
```
...
#   mutate(subj=1:n(), .before=1) %>% # Add temp subj number for simulated data
#   pivot_longer(cols=2:last_col(), names_to=c("Voice_Number", "Question_Number", "Question_Part_Number", NA), names_sep="_", values_to = "responses") %>%
#   full_join(q_labels, .)
# test_data
```
...
# test_data %>%  group_by(subj, Voice_Number) %>%  count() # 29
# test_data %>%  group_by(subj, Voice_Number, Question_Number) %>%  count()
# test_data %>%  group_by(subj, Voice_Number, Question_Number, Question_Part_Number) %>%  count()
```
...
# test_data_clean %>%  group_by(subj, Voice_Number) %>% count() %>% type_convert() %>% arrange(subj, Voice_Number) # 29
# test_data_clean %>% group_by(Voice_Number)%>% count() %>% type_convert() %>% arrange(Voice_Number)
# 
...
# test_data_clean %>% select(subj, Voice_Number) %>% distinct() %>% count()
# # Total presented/answered photos per participant (process/end goal: 10 per subject)
# test_data_clean %>% select(subj, Voice_Number) %>% distinct() %>% count(subj)
# # Total presented/answered participants per photo (end goal: 20 per photo)
# test_data_clean %>% select(subj, Voice_Number) %>% distinct() %>% count(Voice_Number) %>% type_convert() %>% arrange(Voice_Number)

```

```{r}
# test_data_clean %>% filter(subj==1) %>% filter(Voice_Number==7)
```
```{r}
# test_data_clean #%>% type_convert() %>% left_join(n2_audio_codes)
# test_data_clean %>% separate(Question_Number, into=c("Question_Number", "List.Version"), sep="-") %>% rename(Recode_Value=Question_Part_Number) %>% type_convert() %>% left_join(n2_photosel_codes) %>% filter(Question_Number=="Q3.15")
```
...
n2_prolific_demo <- 
  plyr::ldply(pilot_prolific, read_csv, na= c("", "NA", "<NA>	", "DATA_EXPIRED"), show_col_types = FALSE) %>% mutate(`Reviewed at` = as.character(`Reviewed at`)) %>% mutate(Run=RUN_LIST[1]) %>%
  full_join(plyr::ldply(main_prolific, read_csv, na= c("", "NA", "<NA>	", "DATA_EXPIRED"), show_col_types = FALSE) %>% mutate(`Reviewed at` = as.character(`Reviewed at`)) %>% mutate(Run=RUN_LIST[2])) %>%
  full_join(plyr::ldply(final_prolific, read_csv, na= c("", "NA", "<NA>	", "DATA_EXPIRED"), show_col_types = FALSE) %>% mutate(`Reviewed at` = as.character(`Reviewed at`)) %>% mutate(Run=RUN_LIST[3])) %>%
  mutate(Time_in_min=`Time taken`/60, .after=`Time taken`)  
n2_prolific_demo
```

```{r}
n2_screener_data <- read_csv(current_screener) %>%
# Remove metadata columns (first several)
  select(-StartDate:-UserLanguage) %>%
  # Remove unneccessary question header rows (1,2)
  slice(-1:-2) %>%
  unite("Ethnicity", Ethnicity_9:Ethnicity_14, sep=",", na.rm=TRUE, remove=FALSE) %>%
relocate(c(Device, Headphones), .after = Ethnicity) %>%
  rename(`Participant id` = Prolific_ID) %>%
  drop_na(State_Current)

  #TEMP: Remove returned/non-consent participants, bad participants, outliers while checking, before deleting
# n2_screener_data <- n2_screener_data %>% filter(!(PROLIFIC_PID%in% c("62a15df1dd6fa394ea04f063"))) <- REPLACE if needed
...
n2_screener_v_prolific <-
  n2_screener_data %>% full_join(n2_prolific_demo, by="Participant id")  %>% 
  select(
    Run,
    `Participant id`,
    Time_in_min,
...
n2_screener_v_prolific_NA <- n2_screener_v_prolific %>% filter(is.na(Run))
n2_screener_v_prolific_NA

n2_screener_v_prolific <- n2_screener_v_prolific %>%  drop_na(Run)
n2_screener_v_prolific
```
...
n2_screener_v_prolific %>% count(`Participant id`) %>% arrange(desc(n))
```
...
n2_screener_v_prolific %>%  count(`Current u.s state of residence`, `U.s state/territory of birth`)
n2_screener_v_prolific %>%  count(State_Current,State_Born)
```
#### Ethnicity
```{r}
# Sample Counts: Ethnicity
n2_screener_v_prolific %>%  count(Ethnicity_prolific)
n2_screener_v_prolific %>%  count(Ethnicity_screen)
```

#### Device
```{r}
# Sample Counts: Device
n2_screener_v_prolific %>%  count(Device)
```

####  (*C) By-Participant Check
```{r}
participant_list <- n2_screener_v_prolific %>% filter(Run==CURRENT_RUN) %>%  pull(`Participant id`)
participant_list
```

```{r}
current_participant <- "63d842d3e9626bd5fe5c17b1" #participant_list[1] #"5f7d4a2c5db79d21c7d07240"

n2_screener_data %>%  filter(PROLIFIC_PID == current_participant) #Prolific_ID

n2_screener_v_prolific %>%  filter(`Participant id` == current_participant) #Prolific_ID
```
...
n2_data <- read_csv(current_data_num)  %>% 
  filter(Finished!=0) %>%
...
# Use text data for subject data
n2_data_text <- read_csv(current_data_text)  %>% 
  # NOTE: Unfinished data doesn't appear in the export file until X days later. That's why the data may seem to change if I export new data later.
  filter(Finished!="False") %>%
...
```{r}
# Read in audio decoding file
n2_audio_codes <- read.csv(file.path(stim_data_path, NAME, "05-records","SpAAC Speakers & Auditory Stimuli - N2_Stims.csv")) %>% left_join(
  read.csv(file.path(stim_data_path, NAME, "05-records","AAC Speech Speaker Information Survey (Responses) - Recoded Responses.csv")) %>% rename(Code = Speaker_Code) %>% separate(Consent_Uses, into=c("Consent_Uses_1", "Consent_Uses_2", "Consent_Uses_3", "Consent_Uses_4", "Consent_Uses_5", "Consent_Uses_6", "Consent_Uses_7"), sep=", ")
)
n2_audio_codes

# Read in Photo selection List values
n2_photosel_codes <- read.csv(file.path(stim_data_path, NAME, "05-records","SpAAC Visual Stimuli - N2_PseudoRanStim_V2.csv")) %>% select(-List.letter:-Version.number)
n2_photosel_codes

# Extract in Photo decoding values
n2_photo_codes <- n2_photosel_codes %>% select(
  Persona_Group, #Subset_Number, 
  Persona_Code, Condition, Photo_Number, Critical.Filler, 
  Code, Qualtrics.Library.URL, Qualtrics.Image.Link, Qualtrics.Library.ID) %>% distinct()
n2_photo_codes
```
...
```{r}
n2_subj_cond <- n2_subj %>% 
  select(PROLIFIC_PID, VERSION) %>%
  full_join(
    # Get randomized images order list per subject
    n2_data %>%
    select(PROLIFIC_PID, contains("Q2.X_DO")) %>% slice(-1) %>%
    pivot_longer(cols=2:last_col(), names_to=c(NA, NA, "Voice_Number"),
      ...
      pivot_wider(PROLIFIC_PID, names_from = DO, names_prefix = "Trial",
                  values_from = Voice_Number)
  )
n2_subj_cond 

## UNCOMMENT to check a participant
# n2_subj_cond %>%  filter(PROLIFIC_PID=="596fc4717008ef000109703d")
```
...
```{r}
# View
(n2_subj_p1 <- n2_subj %>% select(PROLIFIC_PID, WhatTaskAbout:TaskNotice))
```
Process Part 2 responses about the personal demographics.
```{r}
n2_subj_p2 <- n2_subj %>% select(PROLIFIC_PID, Age:VERSION) %>%
  # Convert -99 to NAs
  ...
  # reshape MixedCat columns
  mutate(across(starts_with("MixedCat"), ~gsub("0", NA, .x))) %>% 
  unite(MixedCat, starts_with("MixedCat"), sep="; ", na.rm=TRUE) %>% 
  mutate(MixedCat = gsub("^$", NA, MixedCat)) %>%
  janitor::remove_empty(which = "cols") # remove columns with only NA
n2_subj_p2
```
...
n2_subj_p2 %>% add_count(name="total_n") %>% count(VERSION, total_n, name="group_n") %>% 
  mutate(prop = round(group_n/total_n,2))
```
##### Age
```{r}
# Age
n2_subj_p2 %>% quick_summarize(Age)
```
```{r}
# Visualize Age
n2_subj_p2 %>%
...
```{r}
# Gender overall
n2_subj_p2 %>% add_count(name="total_n") %>% count(Gender, GenderCat, total_n, name="group_n") %>% 
  mutate(prop = round(group_n/total_n,2))
```
...
# n2_subj_p2_loc # <- UNCOMMENT to view
```
...
for (i in 1:length(participant_list)){
  ...
  # print(n2_subj_p2 %>% filter(PROLIFIC_PID == current_participant))
  
  # View locations by subject
  print(
    n2_subj_p2_loc %>%  filter(PROLIFIC_PID == current_participant) %>%
...
n2_subj_p2 %>% filter(PROLIFIC_PID == current_participant)
...
All participants were living in California* at the time. Most were additionally born in California or spent the majority of their time before age 18 in California (n=2 declined to provide details; n=2 lived only six years of childhood in California).
```{r}
...
##### Network
```{r}
n2_subj_p2_network <- n2_subj_p2 %>% select(PROLIFIC_PID, contains("Community") | contains("Contacts")) %>%
  mutate(across(everything(), as.character)) %>%
...
n2_subj_p2_network <- n2_subj_p2_network %>%
  full_join(n2_subj_p2_network %>% group_by(PROLIFIC_PID) %>% summarize(total_CloseContacts = sum(CloseContacts, na.rm=TRUE))
  ) %>%
...
summary(n2_subj_p2_network)
```

```{r}
# View per Ethnicity data summary
# Q1 -- selections for SocialCommunityNetwork (i.e., all undropped rows)
n2_subj_p2_network %>% count(Ethnicity)
n2_subj_p2_network %>% add_count(PROLIFIC_PID) %>% count(PROLIFIC_PID,n,  Ethnicity) %>% select(-nn)

# Q3 -- CloseContactsLocal
n2_subj_p2_network %>% group_by(Ethnicity) %>% quick_summarize(prop_CloseContactsLocal, na.rm=TRUE)

# Q2 -- CloseContacts
n2_subj_p2_network %>% group_by(Ethnicity) %>% quick_summarize(prop_CloseContacts, na.rm=TRUE)

# Q4 -- Regular Contacts
n2_subj_p2_network %>% group_by(Ethnicity) %>% quick_summarize(prop_RegularContacts, na.rm=TRUE)

# Q5 -- BroaderCommunity
n2_subj_p2_network %>% group_by(Ethnicity) %>% quick_summarize(prop_BroaderCommunity, na.rm=TRUE)
```

```{r}
# Get Asian-specific experience
# Filter to only East and Southeast Asian experience
n2_subj_p2_asian <- n2_subj_p2_network %>% filter(Ethnicity=="East Asian"|Ethnicity=="Southeast Asian") %>%
  mutate(n_CloseContactsLocal = n_CloseContacts*prop_CloseContactsLocal, .after=n_CloseContacts)
n2_subj_p2_asian

# Total N for Asian (absolute)
n2_subj_p2_asian %>% group_by(PROLIFIC_PID) %>% summarize(n_CloseContacts = sum(n_CloseContacts), n_CloseContactsLocal = round(sum(n_CloseContactsLocal)))

# Total prop for Asian (relative)
n2_asian_network <- n2_subj_p2_asian %>%
...
```{r}
# Get sample summary
n2_asian_network %>% summarize(across(where(is.numeric), mean))

# # Q3 -- CloseContactsLocal
n2_asian_network  %>% quick_summarize(prop_AsianLocalCC, na.rm=TRUE)
n2_asian_network  %>% quick_summarize(prop_AsianCC, na.rm=TRUE)
n2_asian_network  %>% quick_summarize(prop_AsianRC, na.rm=TRUE)
n2_asian_network  %>% quick_summarize(prop_AsianBC, na.rm=TRUE)
n2_asian_network  %>% quick_summarize(prop_Asian, na.rm=TRUE)

# Visualize (WIP)
# n2_asian_network %>% pivot_longer() %>%
#   ggplot(aes(x=prop_AsianCC)) + geom_boxplot()
```

```{r}
# Mean Bar plots
n2_asian_network_long <- n2_asian_network %>% pivot_longer(2:last_col(), names_to = "measure", values_to = "proportion", names_prefix = "prop_") %>%
  mutate(measure = fct_relevel(measure, "Asian", "AsianCC", "AsianRC", "AsianBC"))

n2_asian_network_long %>% filter(measure!="AsianLocalCC") %>%
  summary_mean_plot(x=measure, y=proportion, full_scale = TRUE, scale_steps = 0.25) + labs(title="Overall Asian Network/Experience")

n2_asian_network_long %>% filter(measure %in% c("AsianCC", "AsianLocalCC")) %>%
  summary_mean_plot(x=measure, y=proportion, full_scale = TRUE, scale_steps = 0.25) + labs(title="Overall Asian Network/Experience")
```
```{r}
# Get correlation
with(n2_asian_network, 
  cor.test(x=prop_AsianCC, y=prop_AsianRC, method=c ("pearson"))
)

with(n2_asian_network, 
  cor.test(x=prop_AsianCC, y=prop_AsianBC, method=c ("pearson"))
)

with(n2_asian_network, 
  cor.test(x=prop_AsianRC, y=prop_AsianBC, method=c ("pearson"))
)
```
...
#### Merge Data
```{r}
# Merge relevant subject data
n2_subj_info <- n2_subj_p2 %>% 
  # Remove Wide format columns (see excluded multirow data below)
  select(!matches("^(?:Locations|SocialCommunityNetwork|CloseContacts|RegularContacts|BroaderCommunity)")) %>%
  # Add in asian network scores, filling NA with zeros
   full_join(n2_asian_network) %>% mutate(across(starts_with("prop"), ~ ifelse(is.na(.x), 0, .x)))
n2_subj_info
save(n2_subj_info, file=file.path(pipeline, "store", "n2_subj_info.RData"))

# NOTE: There is additional multirow per-participant dataframes (excluded from merging)
# n2_subj_p2_loc # Location
# n2_subj_p2_network # Network scores across all ethnicities
```
...
n2_response_info <-  n2_data %>%
  select(contains("_Q3.")) %>%   select(-contains("DO")) %>% # select only looped trials, excl, DisplayOrder
     slice(1) %>%
  pivot_longer(cols=everything(), names_to=c("Voice_Number", "Question_Number", "Question_Part_Number", NA), names_sep="_", values_to = "q_label") %>%
  # Since number of dashes and <> location aren't consistent, need to remove everything between https:// and extra -
  mutate(q_label = gsub("https://.*? - ", "", q_label)) %>%
  mutate(q_label = gsub("(-\\s*)$", "", q_label)) %>%
  mutate(q_label = gsub("^(\\s*-\\s*)", "", q_label)) %>%
mutate(q_label = gsub("(-\\s+.*\\s+-)", "-", q_label)) %>%
  separate(q_label, into= c("Question", "Question_Part"), sep = "-", extra="merge")
n2_response_info
...
# Get question data + merge with informative question labels
n2_responses <- n2_data %>%
  select("PROLIFIC_PID", contains("_Q3.")) %>%   select(-contains("DO")) %>% # select only looped trials, excl, DisplayOrder
  slice(-1) %>% # remove second header
  pivot_longer(cols=2:last_col(), names_to=c("Voice_Number", "Question_Number", "Question_Part_Number", NA), names_sep="_", values_to = "Response") %>%
  full_join(n2_response_info, .) %>% relocate(PROLIFIC_PID) %>%
  ...
  left_join(., n2_audio_codes) %>% relocate(Voice=Code, .after=Response) %>%
  select(-Qualtrics.Library.ID:-Consent_Uses_7) %>%
  mutate(across(where(is.character), as.factor))
n2_responses
summary(n2_responses)
# View(n2_responses)
save(n2_responses, file=file.path(pipeline, "store", "n2_responses.RData"))
...
n2_responses_selected <- n2_responses %>%
  select(PROLIFIC_PID, Question, Question_Part, Voice, Voice_Number, Response) %>%
  mutate(across(where(is.character), as.factor)) %>%
  mutate(Response = case_when(Question=="Recognize" & Response=="2" ~ "No",
                              Question=="Recognize" & Response=="1" ~ "Yes",
                              TRUE ~ Response)) 
n2_responses_selected
# View(n2_responses_selected)
save(n2_responses_selected, file=file.path(pipeline, "store", "n2_responses_selected.RData"))
```
...
```{r}
# E.g., filter(is.na(Image)) ; filter(Response==-99); filter(Question == "Ratings")
n2_responses %>% filter(is.na(Voice_Number))

# n2_responses_selected %>% filter(Question == "Ratings")  %>% count(PROLIFIC_PID) %>% arrange(n)
# 
# # View one participant/slice
# n2_responses_selected %>%  filter(PROLIFIC_PID=="6366fd2bd9ad6300b4cbc7df") %>% filter(Question=="Ratings") %>% count(Voice_Number, Image)
  # View()
```
...
# Total presented/answered photos (end goal: 600)
n2_responses %>% select(PROLIFIC_PID, Voice_Number) %>% distinct() %>% count()
# Total presented/answered photos per participant (process/end goal: 10 per subject)
n2_responses %>% select(PROLIFIC_PID, Voice_Number) %>% distinct() %>% count(PROLIFIC_PID)
# Total presented/answered participants per photo (end goal: 20 per photo)
n2_responses %>% select(PROLIFIC_PID, Voice_Number) %>% distinct() %>% count(Voice_Number) %>% arrange(Voice_Number)

```
```{r}
# Conditions, sample sizes, sample means, time taken, outliers(?)
# summary(n2_responses)
summary(n2_responses_selected)
```
...
```{r}
current_participant <- "63d228d74dc841d4559ad87d" #participant_list[10] # 

current_responses <- n2_responses %>% 
...
current_responses %>% filter(Question=="Impressions") %>% pivot_wider(id_cols = c(PROLIFIC_PID, Voice), names_from = Question_Part, values_from = Response)

# Non-numeric responses
current_responses %>% filter(Question=="Age") %>%
  filter(Response!=0) %>% select(PROLIFIC_PID, Voice, Question_Part, Response)  %>%
  pivot_wider(id_cols = c(PROLIFIC_PID, Voice), names_from = Question_Part, values_from = Response)

current_responses %>% filter(Question=="GrewUp") %>% select(-contains("Question_Part"), -contains("Question_Number"))
current_responses %>% filter(Question=="Ethnicity") %>% select(-contains("Question_Part"), -contains("Question_Number"))
current_responses %>% filter(Question=="Speech") %>% select(-contains("Question_Part"), -contains("Question_Number"))
...
current_ratings %>%
  ...
  select(Voice, Scale, Response) %>% pivot_wider(id_cols = Voice, names_from = Scale, values_from = Response)

current_ratings %>% summary()
current_ratings %>% group_by(Voice) %>% quick_summarize(Response, na.rm=TRUE)
...
current_likelihood <- current_responses %>% filter(Question=="Likelihood") %>% select(-"Question") %>% select(-PROLIFIC_PID) %>%
  mutate(Scale=str_extract(Question_Part, "(?<=(in\\s|or\\s)).+(?=$)")) %>%  
  mutate(Scale = mgsub(Scale, c("the ", "US or in Canada"), c("", "elsewhere US/Canada"))) %>% 
  mutate(Scale_Group=str_extract(Question_Part, "(?<=)Grew up")) %>%  mutate(Scale_Group=replace(Scale_Group, is.na(Scale_Group), "Race")) %>%
  type_convert()

current_likelihood %>% summary()

# current_likelihood %>%
#   filter(Scale_Group == str_match(Scale_Group, "Grew up")) %>%
#   select(Voice, Scale, Response) %>% pivot_wider(id_cols = Voice, names_from = Scale, values_from = Response)
current_likelihood %>% filter(Scale_Group == "Grew up") %>% group_by(Voice) %>% quick_summarize(Response, na.rm=TRUE)
current_likelihood %>% filter(Scale_Group == "Grew up") %>% group_by(Scale) %>% quick_summarize(Response, na.rm=TRUE)

# current_likelihood %>%
#   filter(Scale_Group == str_match(Scale_Group, "Race")) %>%
#   select(Voice, Scale, Response) %>% pivot_wider(id_cols = Voice, names_from = Scale, values_from = Response) 
current_likelihood %>% filter(Scale_Group == "Race") %>% group_by(Voice) %>% quick_summarize(Response, na.rm=TRUE)
current_likelihood %>% filter(Scale_Group == "Race") %>% group_by(Scale) %>% quick_summarize(Response, na.rm=TRUE)

# Face responses
current_responses %>% filter(Question=="Face") %>% select(-contains("Question_Part"))
