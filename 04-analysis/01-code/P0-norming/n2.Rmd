...
n2_data %>%
  ...
  pivot_longer(cols=2:last_col(), names_to=c(NA, NA, "Audio_Number"), names_sep="_", values_to = "DO") %>% type.convert() %>%
  arrange(subj, DO) %>% drop_na(DO) %>%
  pivot_wider(subj, names_from = DO, names_prefix = "Trial", values_from = Audio_Number)
...
q_labels <-
  ...
  select(contains("_Q3.")) %>%   select(-contains("DO")) %>% # select only looped trials, excl, DisplayOrder
     slice(1) %>%
  pivot_longer(cols=everything(), names_to=c("Audio_Number", "Question_Number", "Question_Part_Number", NA), names_sep="_", values_to = "q_label") %>%
...
test_data <- n2_data %>%
  ...
  mutate(subj=1:n(), .before=1) %>% # Add temp subj number for simulated data
  pivot_longer(cols=2:last_col(), names_to=c("Audio_Number", "Question_Number", "Question_Part_Number", NA), names_sep="_", values_to = "responses") %>%
  full_join(q_labels, .)
test_data
```
...
test_data %>%  group_by(subj, Audio_Number) %>%  count() # 29
test_data %>%  group_by(subj, Audio_Number, Question_Number) %>%  count()
test_data %>%  group_by(subj, Audio_Number, Question_Number, Question_Part_Number) %>%  count()
```
```{r}
...
test_data_clean %>%  group_by(subj, Audio_Number) %>% count() %>% type_convert() %>% arrange(subj, Audio_Number) # 29
test_data_clean %>% group_by(Audio_Number)%>% count() %>% type_convert() %>% arrange(Audio_Number)
...
test_data_clean %>% select(subj, Audio_Number) %>% distinct() %>% count()
# Total presented/answered photos per participant (process/end goal: 10 per subject)
test_data_clean %>% select(subj, Audio_Number) %>% distinct() %>% count(subj)
# Total presented/answered participants per photo (end goal: 20 per photo)
test_data_clean %>% select(subj, Audio_Number) %>% distinct() %>% count(Audio_Number) %>% type_convert() %>% arrange(Audio_Number)

```

```{r}
test_data_clean %>% filter(subj==1) %>% filter(Audio_Number==7)
```
```{r}
...
```{r}
current_participant <- "63d41183860131e49a85cfde" #participant_list[1] #"5f7d4a2c5db79d21c7d07240"
...
n2_subj_cond <- n2_subj %>% 
  ...
  full_join(
    ...
    pivot_longer(cols=2:last_col(), names_to=c(NA, NA, "Audio_Number"),
...
# 63d41183860131e49a85cfde -- born in Philadelphia, grew up in CA since age 2
```
...
n2_responses <- n2_data %>%
  ...
  slice(-1) %>% # remove second header
  pivot_longer(cols=2:last_col(), names_to=c("Audio_Number", "Question_Number", "Question_Part_Number", NA), names_sep="_", values_to = "Response") %>%
  ...
  full_join(., n2_codes) %>% select(-Code) %>%
...
n2_responses_selected <- n2_responses %>%
  select(PROLIFIC_PID, Question, Question_Part, Condition, Image_Cat, Image, Audio_Number, Response) %>%
  mutate(across(where(is.character), as.factor)) %>%
  mutate(Response = case_when(Question=="Recognize" & Response=="2" ~ "No",
                              Question=="Recognize" & Response=="1" ~ "Yes",
                              TRUE ~ Response)) 
...
# n2_responses %>% filter(Question == "Ratings")
# 
# n2_responses_selected %>% filter(Question == "Ratings")  %>% count(PROLIFIC_PID) %>% arrange(n)
# 
# # View one participant/slice
# n2_responses_selected %>%  filter(PROLIFIC_PID=="6366fd2bd9ad6300b4cbc7df") %>% filter(Question=="Ratings") %>% count(Audio_Number, Image)
  # View()
```
...
# Total presented/answered photos (end goal: 600)
n2_responses %>% select(PROLIFIC_PID, Audio_Number) %>% distinct() %>% count()
# Total presented/answered photos per participant (process/end goal: 10 per subject)
n2_responses %>% select(PROLIFIC_PID, Audio_Number) %>% distinct() %>% count(PROLIFIC_PID)
# Total presented/answered participants per photo (end goal: 20 per photo)
n2_responses %>% select(PROLIFIC_PID, Audio_Number) %>% distinct() %>% count(Audio_Number) %>% arrange(Audio_Number)
...
```{r}
current_participant <- "5f1b1f6a908b2609a62ab7ad" #participant_list[10] # 
