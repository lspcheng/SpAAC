...
n1b_bysubj_data <- 
  n1b_subj_p2 %>% select(!matches("^(?:MixedCat|Locations|SocialCommunityNetwork|CloseContacts|RegularContacts|BroaderCommunity)")) %>%  full_join(n1b_asian_network) %>%
  full_join(
...
n1b_byimage_data %>% labeled_scatterplot(x=perceived_american_raised, y=perceived_american_aligned, label=Image, group=Condition, show_points=FALSE)
```
...
n1b_msAsian <- 
  ...
  # perceived as american-raised > 0 (possibly close to ~50% of the time) + top 5 most Asian-aligned
  filter(perceived_american_raised > 0) %>%  slice_min(perceived_american_diff, n=5)
n1b_msAsian

n1b_ethAsian <- 
  n1b_byimage_data %>% filter(Condition=="ethAsian") %>% 
  # perceived as Asian-raised < 0 + top 5 most American-aligned
  filter(perceived_american_raised < 0) %>% slice_max(perceived_american_aligned, n=5)
n1b_ethAsian

n1b_msWhite1 <- 
  n1b_byimage_data %>% filter(Condition=="msWhite") %>% 
  # top 5 most American-aligned
  slice_max(perceived_american_aligned, n=5)
n1b_msWhite1

n1b_msWhite2 <- 
  n1b_byimage_data %>% filter(Condition=="msWhite") %>% 
  # top 5 least American-aligned
  slice_min(perceived_american_aligned, n=5)
n1b_msWhite2 %>% pull(Image)

n1b_selected_images <-  n1b_msAsian %>% rbind(n1b_ethAsian) %>% rbind(n1b_msWhite1)
n1b_selected_images
```
...
#   n1b_byimage_data %>% mutate(
#     Persona_Group = case_when(
#       Image %in% c("AAW-4", "AAW-27", "AAW-63", "AAW-40") ~ "ethAsian",
#       Image %in% c("AAW-47", "AAW-30", "AAW-36", "AAW-45") ~ "msAsian",
#       Image %in% c("WAW-16", "WAW-12", "WAW-58", "WAW-56") ~ "msWhite_1",
#       Image %in% c("WAW-57", "WAW-51", "WAW-41", "WAW-1") ~ "msWhite_2",
#       TRUE ~ NA
#     )
#   ) %>% filter(!is.na(Persona_Group))
# n1b_selected_images
...
n1b_selected_dimensions <-
  n1b_selected_images %>% pivot_longer(Dim1:Dim3, names_to = "Dimension", values_to = "Score") 
...
n1b_selected_composites %>% 
  parallel_plot_color(x=Percept, y=Score, group=Image, color_group=Persona_Group, angle_axis_labels=FALSE) 
```
...
# Composite Americanness
n1b_selected_composites %>%
  parallel_plot(x=Percept, y=Score, group=Persona_Group, angle_axis_labels=FALSE) + labs(title="Persona Groups")
...
# Composite Americanness
n1b_selected_composites %>%
  parallel_plot(x=Percept, y=Score, group=Condition, angle_axis_labels=FALSE) + labs(title="Condition")
```
...
```{r}
# (1) LM summary, (2) tidy LM, (3) EMMs w/ CIs, (4) Pairwise comp, (5) Effect sizes*, (6) EMM comp plot, (7) EMM CI plot
# lm_emms_pairs_condition(n1b_ratings_vars_pca, Dim1 ~ Condition)
