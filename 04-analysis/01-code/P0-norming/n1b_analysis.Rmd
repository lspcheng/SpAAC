...
# Process Main Data
```{r}
load(file.path(PROJECT_DIR, '04-analysis', '02-pipeline', PHASE, "n1b", "store", "n1b_responses.RData"))
load(file.path(PROJECT_DIR, '04-analysis', '02-pipeline', PHASE, "n1b", "store", "n1b_responses_selected.RData"))
load(file.path(PROJECT_DIR, '04-analysis', '02-pipeline', PHASE, "n1b", "store", "n1b_subj_info.RData"))
```
...
n1b_age_byImage <- n1b_age_byImage %>%
  full_join(
    # Get age consensus based on majority decision
   n1b_age_byImage %>% pivot_wider(Image:n, names_from = Age, values_from = prop) %>% 
...
n1b_grewup <-
  ...
  mutate(Response = mgsub(Response, c("philipinnes|phillippines", "^america$|^us$|(?:^|\\W)in the us$|^united states$|^u\\.s\\.$", "(?:^|\\W)(ca|cali|califonia)(?:$|\\W)", "(?:^|\\W)ny(?:$|\\W)", "(?:^|\\W)az(?:$|\\W)", "wisconson"), c("philippines", "usa", "california", "new york", "arizona", "wisconsin"))) %>% 
  ...
  mutate(Region = case_when(
    ...
    str_detect(Response, "(?:^|\\W)(mid(\\s*|-*)west|pacific northwest|(east|west)\\s*coast|(the )*west|(west|east)ern states)(?:$|\\W)|american suburbia|(?:^|\\W)united states(?:$|\\W)|(?:^|\\W)(in|north|northeastern) america(?:$|\\W)") ~ "USA", 
...
n1b_ethnicity <-
  ...
  mutate(Response = mgsub(Response, c("china|chiense", "flippino|philipino|philipno|pilipino|from the philippines", "portugese"), c("chinese", "filipino", "portuguese"))) %>% 
...
# Add categorization into accentedness
  ...
  mutate(Accented = case_when(
    str_detect(Response, "(?:^|\\W)(accented|(with|have) a(.+\\s)accent|(slight|strong|aapi|hawaiian|asian|foreign|heritage('s)*|chinese) accent|(broken|poor(-ish)*) english)(?:$|\\W)") ~ "Accented",
    TRUE ~ NA
  )) %>%
...
n1b_speech <- n1b_speech %>%
  ...
  mutate(Style = case_when(
    Response_Cat == "Other" & str_detect(Response, "(?:^|\\W)(soft|softly|soft spoken|quiet|mild|high|sweet)") ~ "Delicate", # possibly polite
    ...
    Response_Cat == "Other" & str_detect(Response, "(?:^|\\W)(nasal(ly)*)") ~ "Nasal",
    TRUE ~ NA
  ))
...
n1b_speech %>% filter(Style == "Nasal") %>% count(Style, Response, sort=TRUE)
...
```
```{r}
n1b_speech %>% filter(is.na(Style)) %>% count(Style, Response, sort=TRUE)
```
...
```{r}
# n1b_subj_p2_network # add in maybe

n1b_bysubj_data <- 
  n1b_subj_info %>% 
  full_join(
    n1b_descriptors %>% rename(descriptors_response = Response)
  ) %>% 
  full_join(n1b_age) %>%
  full_join(
    n1b_grewup %>% select(-Question) %>% rename(grewup_response = Response, grewup_cat = Response_Cat)
  ) %>%
  full_join(
    n1b_ethnicity %>% select(-Question) %>% rename(ethnicity_response = Response, ethnicity_cat = Response_Cat)
  ) %>%
  full_join(
    n1b_speech %>% select(-Question) %>% rename(speech_response = Response, speech_cat = Response_Cat)
  ) %>% 
  full_join(n1b_ratings) 

# check data
n1b_bysubj_data
colnames(n1b_bysubj_data)
```
```{r}
# Get plot functions: Numeric
target_plot_num <- function(df, grouping_column, current_column, angle_axis_labels=FALSE){
  plt_df <- df %>% select(PROLIFIC_PID:Photo_Number, !!current_column) %>% distinct()
...
target_plot_cat <- function(df, grouping_column, current_column, angle_axis_labels=FALSE){
  ...
  plt_df <- df %>% select(PROLIFIC_PID:Photo_Number, !!current_column) %>% distinct()
  plt_df <- plt_df %>% group_by(!!grouping_column, Condition, Image) %>% add_count(!!grouping_column) 
...
n1b_bysubj_data_gender <- n1b_bysubj_data %>% filter(GenderCat!="Prefer not to answer") %>% mutate(GenderCat = gsub("^Non.*", "NB", GenderCat)) %>% mutate(GenderCat = fct_relevel(GenderCat, "Man", "Woman", "NB")) %>% group_by(GenderCat)  %>% 
  # reorder factors
  mutate(Rating_Scale = fct_relevel(Rating_Scale,
...
current_column <- "Age"
# Summary Table
quick_summarize(n1b_bysubj_data_gender, current_column, na.rm=TRUE)
# Summary Plot
target_plot_num(n1b_bysubj_data_gender, grouping_column, current_column)
```
...
# Summary Plot
target_plot_cat(n1b_bysubj_data_gender, grouping_column, current_column, angle_axis_labels=TRUE)
```
```{r}
...
# Summary Plot
n1b_bysubj_data_gender %>% select(PROLIFIC_PID:Photo_Number, Rating_Group:Response_ZScore) %>% distinct() %>%
  filter(GenderCat!="NB") %>%
  parallel_plot(x=Rating_Scale, y=Rating_ZScore, group=GenderCat, angle_axis_labels=TRUE, viridis=FALSE)  +
  labs(title="All Conditions")

current_filter <- "ethAsian"
n1b_bysubj_data_gender %>% select(PROLIFIC_PID:Photo_Number, Rating_Group:Response_ZScore) %>% distinct() %>%
  filter(Condition==current_filter)  %>% filter(GenderCat!="NB") %>%
  parallel_plot(x=Rating_Scale, y=Rating_ZScore, group=GenderCat, angle_axis_labels=TRUE, viridis=FALSE)  + labs(title=current_filter)

current_filter <- "msAsian"
n1b_bysubj_data_gender %>% select(PROLIFIC_PID:Photo_Number, Rating_Group:Response_ZScore) %>% distinct() %>%
  filter(Condition==current_filter)  %>% filter(GenderCat!="NB") %>%
  parallel_plot(x=Rating_Scale, y=Rating_ZScore, group=GenderCat, angle_axis_labels=TRUE, viridis=FALSE)  + labs(title=current_filter)

current_filter <- "msWhite"
n1b_bysubj_data_gender %>% select(PROLIFIC_PID:Photo_Number, Rating_Group:Response_ZScore) %>% distinct() %>%
  filter(Condition==current_filter)  %>% filter(GenderCat!="NB") %>%
  parallel_plot(x=Rating_Scale, y=Rating_ZScore, group=GenderCat, angle_axis_labels=TRUE, viridis=FALSE)  + labs(title=current_filter)
```
...
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim1, y=Dim2, label=Image, group=Condition, show_points=TRUE)
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim1, y=Dim3, label=Image, group=Condition, show_points=TRUE)
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim1, y=Dim4, label=Image, group=Condition, show_points=TRUE)

n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim2, y=Dim3, label=Image, group=Condition, show_points=TRUE)
n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim2, y=Dim4, label=Image, group=Condition, show_points=TRUE)

n1b_ratings_vars_pca %>% labeled_scatterplot(x=Dim3, y=Dim4, label=Image, group=Condition, show_points=TRUE)
...
n1b_byimage_data <- n1b_byimage_data %>% 
  mutate(perceived_american_raised = (American + `Native speaker of English` + `American-accented`)/3 ,
        perceived_american_aligned = `Aligned with American culture`, .after=Image)  %>%
  mutate(perceived_american_diff = perceived_american_aligned - perceived_american_raised, .after=Image)
```
...
# Visualize
n1b_byimage_data %>% 
  ggplot(aes(x=perceived_american_raised, y=perceived_american_aligned, color=Condition)) + 
  geom_point() + gg_theme()
...
```{r}
# Get rating colnames
# n1b_ratings_vars_pca %>% select(American:Intelligent) %>% colnames()

# Check for outliers
n1b_byimage_data %>% get_outliers(American) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(`Native speaker of English`) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(`American-accented`) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(`Aligned with American culture`) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Feminine) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(`Clear speaker`) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Cool) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Casual) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Nerdy) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(`Slow speaker`) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Enthusiastic) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Confident) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Friendly) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Likeable) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Attractive) %>% filter(outlier==TRUE)
n1b_byimage_data %>% get_outliers(Intelligent) %>% filter(outlier==TRUE)
# # NOTE: None
```
