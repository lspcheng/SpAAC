...
* Based on not being outliers (mahalanobis scores; 3SD outliers)
...
How do we decide on cut-offs numerically rather than visually?
```{r}
# Select by average rank (WIP)
n1b_byimage_data %>% filter(Condition=="msAsian") %>% 
  mutate(paa_rank = rank(perceived_american_aligned), .after=Image) %>% 
  mutate(par_rank = rank(-perceived_american_raised), .after=Image) %>% 
  mutate(average_rank = (par_rank+paa_rank)/2, .after=Image) %>% arrange(average_rank)
```
...
##### Visualize
```{r}
# Condition Mean Parallel Plots by 
...
n1b_selected_images %>% pivot_longer(American:Intelligent, names_to = "Rating_Scale", values_to = "Rating_ZScore") %>%
  ...
  "Enthusiastic", "Confident", "Friendly", "Likeable", "Attractive", "Intelligent",               # Traits
  "Feminine", "Clear speaker","Cool", "Casual",  "Nerdy", "Slow speaker"))    %>%                      # Style
  parallel_plot(x=Rating_Scale, y=Rating_ZScore, group=Persona_Group, angle_axis_labels=TRUE) 

# Dimensions
n1b_selected_images %>% pivot_longer(Dim1:Dim3, names_to = "Dimension", values_to = "Score") %>%
  parallel_plot(x=Dimension, y=Score, group=Persona_Group, angle_axis_labels=FALSE) 

# Percepts
n1b_selected_images %>% pivot_longer(from_asia_prop:positive_prop, names_to = "Percept", values_to = "Score") %>%
  parallel_plot(x=Percept, y=Score, group=Persona_Group, angle_axis_labels=TRUE) 

# Composite Americanness
n1b_selected_images %>% pivot_longer(perceived_american_raised:perceived_american_aligned, names_to = "Percept", values_to = "Score") %>%
  parallel_plot(x=Percept, y=Score, group=Persona_Group, angle_axis_labels=FALSE) 
